...
void BuildML(int N_BR, int N_ML, double *d_layer, double complex *n, double *rind, double *d);
// Global variables
int polflag;
int alloyflag;
double c = 299792458;
  ...
  for (int i=0; i<1; i++) {
	...
	for (int l=0; l<NumVars; l++) {
	  ...
	  for (int m=0; m<NumVars; m++) {
	    ...
	    // Now W substrate and air underlayer
	    d[10] = 0.9;
	    d[11] = 0.;
	     ...
	     // Now start the muli-layer absorber
 	     ...
 	     // Re Absober
  	     ...
  	     // Al2O3 Dielectric
             ...
             // Now W substrate and air underlayer
             rind[10] = csqrt(substrate_eps[ii]);
...
double PrintSpectrum(char *filename, double *d, double complex *rind, int Nlayer, double d1, double complex *nlow, double d2, double complex *nhi, double d3, double complex *barrier_n, double Temp, 
	    ...
	    d[0] = 0.;
            ...
            // Now W substrate and air underlayer
            d[10] = 0.9;
            d[11] = 0.;	    // Air and alloy thicknesses
